...
class SA_FIGHTER(Env):
    ...
    metadata = {
        "render_modes": ["human"],
        "name": "sarl_fighter_vs_goblins_v01",
    }
    ...
    def step(self, action):
        ...
        info = {"action_mask": fighter_action_mask}
        ...
        return observations, reward, terminations, None, info # CHANGED from done
    ...
    def cleric_turn(self):
        if self.debug_mode:
                print(f"Cleric turn starts ...")
                
        # Move Action (0), Spell Cast Action (1, 2, 3, 4), Cantrip Action (5, 6, 7)
        if self.state["cleric"]["alive"] and self.goblins_alive >= 1:
            
            # 0.1
            if self.state["cleric"]["hp"] <= 5 and self.state["cleric"]["zone"] != 2:
                self.move_action(self.state["cleric"])
                
            # 1, 2, 3, 4
            elif self.state["cleric"]["spellslots"] > 0 and (self.state["cleric"]["hp"] < 5 or self.state["fighter"]["hp"] < 5 or self.state["rogue"]["hp"] < 5 or self.state["wizard"]["hp"] < 5):
                if self.state["cleric"]["hp"] < 5:
                    target = self.state["cleric"]
                    target_name = "cleric"
                elif self.state["fighter"]["hp"] < 5:
                    target = self.state["fighter"]
                    target_name = "fighter"
                elif self.state["rogue"]["hp"] < 5:
                    target = self.state["rogue"]
                    target_name = "rogue"
                elif self.state["wizard"]["hp"] < 5:
            ...
            # 0.2
            elif self.state["cleric"]["hp"] > 5 and self.state["cleric"]["zone"] != 1:
                self.move_action(self.state["cleric"])

            # 5, 6, 7
            else:
                action = random.randint(5, 7)
                if action == 5:
                    mode = "low"
                elif action == 6:
                    mode = "high"
